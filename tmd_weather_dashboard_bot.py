...
function buildDataFromRows(rows) {
  ...
  rows.forEach(row => {
    ...
    if (row.Condition) {
      ...
      conditionMap[stationCode][dateStr][timeStr] = { emoji: emoji, text: text };

      conditionMap[stationRaw] = conditionMap[stationRaw] || {};
      conditionMap[stationRaw][dateStr] = conditionMap[stationRaw][dateStr] || {};
      conditionMap[stationRaw][dateStr][timeStr] = { emoji: emoji, text: text };
    }
...
function attachFluidPhysicsCanvas(canvasId, rainVal, windSpeed) {
  ...
  canvas.width = canvas.offsetWidth || 82;
...
function renderMatrix() {
  ...
  DATA.dates.forEach((d, i) => {
    ...
    dateTd.innerHTML = '<div class="date-title">' + d + '</div><div class="emoji-slots-strip">' + slotEmojis + '</div>';
    ...
    sumTd.innerHTML = '<div class="summary-val">' + (totalRain > 0 ? totalRain + ' mm Total' : '0 mm') + '</div><div class="summary-commentary">' + evalData.commentary + '</div>';
